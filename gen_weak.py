...
def weaken_img(pn: Tuple, strategy: Callable) -> Tuple[int, int]:
    ...
    try:
        assert set(np.unique(img)).issubset(set(range(args.K)))
    ...
    selected_class: int
    filling: int
    to_merge: List[np.ndarray] = []

    # Dirty hack for now -- assume that the 0 class is _always_ background,
    # allowing us to sum all resulting weakening, as a merging strategy.
    for selected_class, filling in zip(args.selected_class, args.filling):
        ...
        to_merge.append(res_arr)

    final_arr: np.ndarray = sum(to_merge)
    final_img: Image = Image.fromarray(final_arr, mode='L')
    ...
    if args.display:
        ...
        grid[0].imshow(img)
        grid[0].set_title("Original labels")
        grid[1].imshow(final_img)
        grid[1].set_title("Weakened labels")
...
def get_args() -> Namespace:
    ...
    parser.add_argument("--selected_class", type=int, required=True, nargs="+",
                        help="Default used to be 3")
    parser.add_argument("--filling", type=int, required=True, nargs="+",
                        help="Default used to be 3")

    parser.add_argument("--verbose", action="store_true")
    parser.add_argument("--display", action="store_true", help="Display _each_ resulting image with matplotlib.")
    ...
    parser.add_argument("-K", type=int, default=2, help="Number of classes")
    ...
    assert len(args.selected_class) == len(args.filling)
    assert 0 not in args.selected_class, "0 is considered background, and cannot be weakened. Consider remapping first"
